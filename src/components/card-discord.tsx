    ...
    <Link href={siteConfig.links.discord} target="_blank" rel="noreferrer">
      <Card className="flex-1 basis-1/3">
        <CardHeader className="space-y-0 pb-2">
          ...
          <CardDescription>Join my Discord community and connect with other software developers</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">
            {loading ? <Skeleton className="h-4 w-full mb-2" /> : `${data?.totalMembers} members`}
          </div>
          <div className="text-xs text-muted-foreground">
            {loading ? <Skeleton className="h-4 w-full" /> : `${data?.onlineMembers} online now`}
          </div>
        </CardContent>
      </Card>
    </Link>
